import React from 'react';
import './HomePage.css';
import logoImage from '../../Images/Cozyfox.gif'

function HomePage() {
  return (
    <div className="home-page">
      <h1>Welcome to The Cozy Fox Den!</h1>
      <p>Your retreat for relaxation and musical indulgence. 
        Here, you're invited to snuggle up and find your comfort zone while enjoying the soothing sounds we offer. 
        Dive into our collection of videos and let them whisk you away into a world of tranquility. 
        And if you're inclined to share your own favorites, why not sign up and join our community? 
        Embrace the cozy atmosphere and let the music take you to new levels of relaxation.
      </p>
      <img src={logoImage} alt="The Cozy Fox Den Logo" className="logo-image" loop="1" />
    </div>
  );
}

export default HomePage;
